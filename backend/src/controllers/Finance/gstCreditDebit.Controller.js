...
const TABLE = 'gst_credit_debit_notes';
const PK = 'note_id';

export const createGstCreditDebit = async (req, res) => {
    try {
        const body = req.body || {};
...
export const getGstCreditDebitById = async (req, res) => {
    try {
        ...
        console.error('Error fetching gst credit/debit by id:', error);
        res.status(500).json({ success: false, message: 'Error fetching record', error: error.message });
    }
};

export const updateGstCreditDebit = async (req, res) => {
    try {
        const { id } = req.params;
        const body = req.body || {};
        const keys = Object.keys(body);
        ...
        console.error('Error updating gst credit/debit:', error);
        res.status(500).json({ success: false, message: 'Error updating record', error: error.message });
    }
};

export const deleteGstCreditDebit = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        console.error('Error deleting gst credit/debit:', error);
