...
const TABLE = 'payable_ageing';
const PK = 'payable_ageing_id';

export const createPayableAgeing = async (req, res) => {
    try {
        const body = req.body || {};
...
export const getPayableAgeingById = async (req, res) => {
    try {
        ...
        console.error('Error fetching payable ageing by id:', error);
        res.status(500).json({ success: false, message: 'Error fetching record', error: error.message });
    }
};

export const updatePayableAgeing = async (req, res) => {
    try {
        const { id } = req.params;
        const body = req.body || {};
        const keys = Object.keys(body);
        ...
        console.error('Error updating payable ageing:', error);
        res.status(500).json({ success: false, message: 'Error updating record', error: error.message });
    }
};

export const deletePayableAgeing = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        console.error('Error deleting payable ageing:', error);
