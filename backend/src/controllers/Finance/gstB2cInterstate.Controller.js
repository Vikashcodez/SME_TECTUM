...
const TABLE = 'gst_b2c_interstate_sales';
const PK = 'interstate_id';

export const createGstB2cInterstate = async (req, res) => {
    try {
        const body = req.body || {};
...
export const getGstB2cInterstateById = async (req, res) => {
    try {
        ...
        console.error('Error fetching gst b2c interstate by id:', error);
        res.status(500).json({ success: false, message: 'Error fetching record', error: error.message });
    }
};

export const updateGstB2cInterstate = async (req, res) => {
    try {
        const { id } = req.params;
        const body = req.body || {};
        const keys = Object.keys(body);
        ...
        console.error('Error updating gst b2c interstate:', error);
        res.status(500).json({ success: false, message: 'Error updating record', error: error.message });
    }
};

export const deleteGstB2cInterstate = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        console.error('Error deleting gst b2c interstate:', error);
