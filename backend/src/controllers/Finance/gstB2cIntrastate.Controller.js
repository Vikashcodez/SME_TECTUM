...
const TABLE = 'gst_b2c_intrastate_sales';
const PK = 'intrastate_id';

export const createGstB2cIntrastate = async (req, res) => {
    try {
        const body = req.body || {};
...
export const getGstB2cIntrastateById = async (req, res) => {
    try {
        ...
        console.error('Error fetching gst b2c intrastate by id:', error);
        res.status(500).json({ success: false, message: 'Error fetching record', error: error.message });
    }
};

export const updateGstB2cIntrastate = async (req, res) => {
    try {
        const { id } = req.params;
        const body = req.body || {};
        const keys = Object.keys(body);
        ...
        console.error('Error updating gst b2c intrastate:', error);
        res.status(500).json({ success: false, message: 'Error updating record', error: error.message });
    }
};

export const deleteGstB2cIntrastate = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        console.error('Error deleting gst b2c intrastate:', error);
