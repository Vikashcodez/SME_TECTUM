...
const TABLE = 'gst_exports';
const PK = 'export_id';

export const createGstExport = async (req, res) => {
    try {
        const body = req.body || {};
...
export const getGstExportById = async (req, res) => {
    try {
        ...
        console.error('Error fetching gst export by id:', error);
        res.status(500).json({ success: false, message: 'Error fetching record', error: error.message });
    }
};

export const updateGstExport = async (req, res) => {
    try {
        const { id } = req.params;
        const body = req.body || {};
        const keys = Object.keys(body);
        ...
        console.error('Error updating gst export:', error);
        res.status(500).json({ success: false, message: 'Error updating record', error: error.message });
    }
};

export const deleteGstExport = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        console.error('Error deleting gst export:', error);
