...
const TABLE = 'gst_advances_received';
const PK = 'advance_id';

export const createGstAdvance = async (req, res) => {
    try {
        const body = req.body || {};
...
export const getGstAdvanceById = async (req, res) => {
    try {
        ...
        console.error('Error fetching gst advance by id:', error);
        res.status(500).json({ success: false, message: 'Error fetching record', error: error.message });
    }
};

export const updateGstAdvance = async (req, res) => {
    try {
        const { id } = req.params;
        const body = req.body || {};
        const keys = Object.keys(body);
        ...
        console.error('Error updating gst advance:', error);
        res.status(500).json({ success: false, message: 'Error updating record', error: error.message });
    }
};

export const deleteGstAdvance = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        console.error('Error deleting gst advance:', error);
